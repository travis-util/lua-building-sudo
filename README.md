# lua-building

* https://luarocks.org/#quick-start
* https://tracker.debian.org/pkg/luarocks
* https://keplerproject.github.io/wsapi/
  * https://keplerproject.github.io/wsapi/manual.html
* https://github.com/keplerproject/wsapi/blob/master/samples/fastcgi-example.lua
* [*Running WSAPI on ngninx via FastCGI*](https://serverfault.com/questions/556007/running-wsapi-on-ngninx-via-fastcgi)
* https://github.com/mwild1/luadbi
  * https://tracker.debian.org/pkg/lua-dbi
